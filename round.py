...
class Round:
    def __init__(self, screen, current_turn=1, max_turns=13, max_rolls=3):
        """Inicjalizuje obiekt rundy z ekranem, liczbą tur i rzutów."""
        self.screen = screen
    ...
    def draw_game_over(self, total_scores):
        """Wyświetla ekran końca gry z wynikami i przyciskiem 'Zagraj ponownie'."""
        ...
        # Wyświetlanie wyników z odpowiednimi kolorami dla miejsc 1-3
    ...
    def scores_to_txt(self, total_scores):
        """Zapisuje wyniki gry do pliku tekstowego wraz z datą."""
    ...
    def cup_animation(self):
        """Wyświetla animację potrząsania kubkiem z kośćmi."""
        total_frames = 6
        images = {}
    ...
    def update_dices(self):
        """Aktualizuje animacje wszystkich kości."""
        for dice in self.dices:
            dice.update()

    def draw_turn_info(self):
        """Wyświetla informację o aktualnej rundzie i liczbie rzutów."""
    ...
    def draw_game(self, scoreboard, player_name, difficulty):
        """Rysuje całą planszę gry, kości, przyciski i informacje o graczu."""
        self.screen.fill(DARK_GREEN)
    ...
    def increase_roll_count(self):
        """Zwiększa licznik rzutów w bieżącej turze."""
        self.roll_count += 1

    def next_turn(self):
        """Przechodzi do następnej tury, resetuje rzuty i kości."""
        self.current_turn += 1
        self.roll_count = 0
        self.roll.show()
        self.reroll.hide()
    ...
    def update_roll_buttons(self):
        """Ukrywa/odkrywa przyciski rzutu w zależności od liczby rzutów."""
        if self.roll.visible and self.roll_count >= self.max_rolls:
            self.roll.hide()
            self.reroll.show()

    def reset(self):
        """Resetuje stan rundy do początkowego."""
        self.roll_count = 0
        for dice in self.dices:
            dice.reset()
        self.roll.show()
    ...
    def hide_dices(self, scoreboard, player_name, difficulty):
        """Ukrywa nie wybrane kości po rzucie."""
        for dice in self.dices:
    ...
    def all_dices_selected(self):
        """Zwraca True jeśli wszystkie kości są zaznaczone."""
        for dice in self.dices:
    ...
    def play_again_clicked(self, event):
        """Sprawdza czy kliknięto przycisk 'Zagraj ponownie'."""
